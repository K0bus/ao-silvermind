...
- **Application Naming**: The application is officially named **Albion SilverMind**. Do not use "Albion Tool". All user-facing branding, layout headers, footers, page titles, documentation, and metadata must use **Albion SilverMind** (or **Albion - SilverMind** in HTML titles/metadata).
- **DockerHub Images**: The Docker images built for deployment must be named `albion-silvermind-app` (for the web app) and `albion-silvermind-worker` (for the background worker).
- **Language Protocol**: All user communication, code comments, and documentation must be in English.
